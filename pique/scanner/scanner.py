        ...
        try:
            for event_container in self.events:
                ...
                while start_block <= latest_block:
                    LOGGER.debug(f"Fetching events from {start_block} to {start_block + self.batch_size}")
                    await self.fetch_events(event_container, start_block, self.batch_size)
                    LOGGER.debug(f"Finished fetching events from {start_block} to {start_block + self.batch_size}")
                    start_block += self.batch_size
                LOGGER.debug(f"Finished scanning {event_container.name}|{event_container.address[:8]}")
